...
module.exports = {
  ...
  async run(m, { sock, text, Func }) {
    ...
    console.log(p);
    if (!p[0].exists) throw "Pengguna tidak terdaftar di WhatsApp"
    const jid = p[0].jid
    const member = m.metadata.participants.find((u) => u.id === jid);
    if (member) {
      return m.reply("Pengguna tersebut sudah menjadi anggota grup ini.");
    }
    ...
    for (let res of resp) {
      if (res.status === 421) {
        m.reply(
          "Tidak dapat menambahkan pengguna tersebut. Mereka telah membatasi undangan ke grup.",
        );
